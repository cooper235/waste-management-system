...
exports.getAllImages = async (req, res, next) => {
  try {
    ...
    logger.error(`[IMAGE] Get all images error: ${error.message}`)
    res.status(500).json({
      success: false,
...
exports.uploadImage = async (req, res, next) => {
  try {
    ...
    if (!bin) {
      ...
      const validCategories = ["metal", "biodegradable", "non-biodegradable", "plastic", "others"];
      const category = validCategories.includes(predictedCategory) ? predictedCategory : "others";
      ...
      await bin.save()
      logger.info(`[IMAGE] Created default bin: ${binId}`)
    }
